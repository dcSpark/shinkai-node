...
impl SubscriberManager {
    pub async fn new(db: Weak<Mutex<ShinkaiDB>>, vector_fs: Weak<Mutex<VectorFS>>) -> Self {
        SubscriberManager { db, vector_fs }
    }

    pub async fn process_action(&mut self, su: ShinkaiMessage) -> Result<String, SomeError> {
...
//         let job_queue = JobQueueManager::<NetworkJobForProcessing>::new(db.clone()).await.unwrap();
...
//         job_queue_manager: Arc<Mutex<JobQueueManager<NetworkJobForProcessing>>>,
//         db: Weak<Mutex<ShinkaiDB>>,
...
//         profile: &ShinkaiName,
//     ) -> Result<String, AgentError> {
//         let job_for_processing = NetworkJobForProcessing::new(job_message.clone(), profile.clone());
