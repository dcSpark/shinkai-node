...
export class ShinkaiMessageBuilderWrapper {
  ...
  internal_metadata(sender_subidentity: string, recipient_subidentity: string, inbox: string, encryption: any): void {
    this.wasmBuilder.internal_metadata(sender_subidentity, recipient_subidentity, encryption);
  }
  ...
  static use_code_registration(my_subidentity_encryption_sk: string, my_subidentity_signature_sk: string, receiver_public_key: string, code: string, identity_type: string, permission_type: string, registration_name: string, sender_profile_name: string, receiver: string): string {
    return ShinkaiMessageBuilderWrapperWASM.use_code_registration(my_subidentity_encryption_sk, my_subidentity_signature_sk, receiver_public_key, code, identity_type, permission_type, registration_name, sender_profile_name, receiver);
  }

  static request_code_registration(my_subidentity_encryption_sk: string, my_subidentity_signature_sk: string, receiver_public_key: string, permission_type: string, code_type: string, sender_profile_name: string, receiver: string): string {
    return ShinkaiMessageBuilderWrapperWASM.request_code_registration(my_subidentity_encryption_sk, my_subidentity_signature_sk, receiver_public_key, permission_type, code_type, sender_profile_name, receiver);
  }
