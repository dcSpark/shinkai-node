...
fn test_localhost_relay_server_communication() {
    ...
    let e: Result<(), tokio::task::JoinError> = rt.block_on(async {
        ...
        let (node2_identity_sk, node2_identity_pk) = unsafe_deterministic_signature_keypair(100);
        let (node2_encryption_sk, node2_encryption_pk) = unsafe_deterministic_encryption_keypair(100);
