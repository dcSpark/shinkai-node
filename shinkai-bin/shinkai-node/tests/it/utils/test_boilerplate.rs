        ...
        match result {
            ...
            Err(e) => {
                ...
                if e.is_cancelled() {
                    ...
                    // Do not panic or assert here as it will be not bubbled up to the test runner
                    return Err(e);
                }
            }
        }
    });
    // First shutdown the runtime to ensure all tasks are finished, then assert - or it will not be bubbled up.
