...
async fn test_vrkai_vrpack_vector_search() {
    ...
    let vrpack_search_results = vrpack
        .dynamic_deep_vector_search(query_string, 100, 100, generator, VectorSearchMode::Default)
        .await
        .unwrap();
...
async fn test_vector_fs_saving_reading() {
    ...
    let query_embedding = vector_fs
        ...
        .unwrap();
    let res = vector_fs
        .deep_vector_search(&reader, query_string.clone(), 100, 100, VectorSearchMode::Default)
        .await
        .unwrap();
    assert_eq!(
