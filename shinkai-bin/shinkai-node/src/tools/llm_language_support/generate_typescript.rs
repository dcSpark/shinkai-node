 ...
 * @param location - (required) The city or location to get weather for
 * @param units - (optional) Temperature units (celsius/fahrenheit), default: undefined
 * @returns The result object
 */
async function getWeather(location: string, units?: string): Promise<GetWeatherResponse> {
    const url = 'http://localhost:9550/v2/tool_execution';
    const data = {
        tool_router_key: 'local:::shinkai-tool-weather:::shinkai__weather',
        parameters: {
            location,
            units,
        },
    };
    const response = await axios.post<GetWeatherResponse>(url, data, {
        headers: {
...
pub fn generate_typescript_definition(
    ...
    if let Some(properties) = runner_def.parameters.get("properties").and_then(|v| v.as_object()) {
        for (param_name, param_value) in properties {
            ...
            typescript_output.push_str(&format!(" * @param {} - ({}{}) {}\n", 
                param_name,
                if required { "required" } else { "optional" },
                if !desc.is_empty() { format!(", {}", desc) } else { String::new() },
    ...
    if let Some(properties) = runner_def.result.get("properties").and_then(|v| v.as_object()) {
        for (prop_name, prop_value) in properties {
            ...
            let desc = prop_value.get("description")
                .and_then(|d| d.as_str())
                .unwrap_or("");
            typescript_output.push_str(&format!(" *   {}: {} {}\n", 
                prop_name, 
    ...
    typescript_output.push_str(&format!("    const _url = 'http://localhost:{}/v2/tool_execution';\n", api_port));
