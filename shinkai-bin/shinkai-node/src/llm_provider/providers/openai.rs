...
use super::shared::openai::{openai_prepare_messages, FunctionCall, MessageContent, OpenAIResponse};
    ...
    while let Some(item) = stream.next().await {
        ...
        match item {
            Ok(chunk) => {
                ...
                eprintln!("New Chunk: {}", chunk_str);

                previous_json_chunk += chunk_str.as_str();
                let trimmed_chunk_str = previous_json_chunk.trim().to_string();
                eprintln!("Accumulated Chunk: {}", trimmed_chunk_str);

                let data_resp: Result<JsonValue, _> = serde_json::from_str(&trimmed_chunk_str);
                match data_resp {
                    Ok(data) => {
                        eprintln!("Data: {}", data);
                        // tool call example using streaming
                        /* {
                            "id":"chatcmpl-AAQpOFH1lAFesehOlSDu1hbNZTkA4",
                            "object":"chat.completion",
                            "created":1727049770,
                            "model":"gpt-4o-mini-2024-07-18",
                            "choices":[{"index":0,"message":{"role":"assistant","content":null,"function_call":{"name":"shinkai__perplexity","arguments":"{\"query\":\"Roman Empire history\"}"},"refusal":null},"logprobs":null,"finish_reason":"function_call"}],
                            "usage":{"prompt_tokens":183,"completion_tokens":21,"total_tokens":204,"completion_tokens_details":{"reasoning_tokens":0}},
