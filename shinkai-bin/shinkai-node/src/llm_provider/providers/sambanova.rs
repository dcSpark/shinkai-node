...
use crate::llm_provider::providers::shared::sambanova_api::sambanova_prepare_messages;
...
use shinkai_message_primitives::schemas::llm_providers::serialized_llm_provider::{SambaNova, LLMProviderInterface};
    ...
    async fn call_api(
        ...
        if let Some(base_url) = url {
            if let Some(key) = api_key {
                ...
                let result = sambanova_prepare_messages(&model, prompt)?;
    ...
    if let Some(other_params) = config.and_then(|c| c.other_model_params.as_ref()) {
        if let Some(obj) = other_params.as_object() {
            for (key, value) in obj {
                match key.as_str() {
                    // SambaNova supports top_k which is not in OpenAI
                    "top_k" => payload["top_k"] = value.clone(),
                    "stop" => payload["stop"] = value.clone(),
                    "response_format" => payload["response_format"] = value.clone(),
                    "max_tokens" => payload["max_tokens"] = value.clone(),
                    _ => (),
                };
            }
        }
