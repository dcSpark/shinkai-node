...
// shinkai_name::{ShinkaiName, ShinkaiSubidentityType}     }, shinkai_message::{
...
// APIReadUpToTimeRequest, IdentityPermissions, MessageSchemaType, RegistrationCodeRequest, RegistrationCodeType
//         }
//     }, shinkai_utils::{
//         encryption::{
//             clone_static_secret_key, encryption_public_key_to_string, string_to_encryption_public_key,
// EncryptionMethod         }, shinkai_logging::{shinkai_log, ShinkaiLogLevel, ShinkaiLogOption},
...
//             Identity::Standard(std_identity) => Self::has_standard_identity_access(db, inbox_name,
// std_identity).await,             Identity::Device(std_device) => Self::has_device_identity_access(db, inbox_name,
// std_device).await,             _ => Err(NodeError {
//                 message: format!(
...
//                 if value {
//                     let response =
//                         Self::internal_get_last_messages_from_inbox(db.clone(), inbox_name.to_string(), count,
// offset)                             .await;
...
//                             error: "Don't have access".to_string(),
//                             message: format!(
//                                 "Permission denied. You don't have enough permissions to see this profile's inboxes
// list: {}",                                 profile_requested
//                             ),
//                         }))
//                         .await;
//                         return Ok(());
...
//         // Convert ShinkaiName to StandardIdentity if necessary and check if it's a Profile type with admin
// privileges         let identity = identity_manager
//             .lock()
//             .await
...
//     pub async fn api_is_pristine(db: Arc<SqliteManager>, res: Sender<Result<bool, APIError>>) -> Result<(),
// NodeError> {         let has_any_profile = db.has_any_profile().unwrap_or(false);
...
//             match inbox {
//                 Ok(inbox) => {
//                     // TODO: extend and verify that the sender may have access to the inbox using the access db
// method                     match inbox.has_sender_creation_access(msg.clone()) {
//                         Ok(_) => {
...
// ShinkaiName::from_shinkai_message_only_using_recipient_node_name(&msg.clone())             .unwrap()
//             .to_string();
