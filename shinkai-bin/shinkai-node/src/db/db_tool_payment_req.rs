...
impl ShinkaiDB {
    pub fn set_tool_offering(&self, tool_offering: ShinkaiToolOffering) -> Result<(), ShinkaiDBError> {
    ...
    pub fn get_tool_offering(&self, tool_key_name: &str) -> Result<ShinkaiToolOffering, ShinkaiDBError> {
        ...
        let tool_offering: ShinkaiToolOffering = serde_json::from_slice(&offering_bytes)
            .map_err(|e| ShinkaiDBError::SomeError(format!("Failed to deserialize tool offering: {:?}", e)))?;
        Ok(tool_offering)
    }
