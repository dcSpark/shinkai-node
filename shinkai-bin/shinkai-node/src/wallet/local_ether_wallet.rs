...
//         let provider =
//             Provider::<Http>::try_from(network.default_rpc()).map_err(|e|
// WalletError::InvalidRpcUrl(e.to_string()))?;
...
//                 let derivation_path =
//                     DerivationPath::from_str("m/44'/60'/0'/0/0").map_err(|e|
// WalletError::Bip39Error(e.to_string()))?;                 let child_xprv = derivation_path
//                     .into_iter()
...
//             .await
//             .map_err(|e| WalletError::ProviderError(e.to_string()))?;
//         let max_priority_fee_per_gas = U256::from(2_000_000_000); // 2 Gwei // TODO: If we delete this, it should
// still work (automatically calculated by the network?)
...
// about";         let wallet =
//             LocalEthersWallet::recover_wallet(network.clone(),
// WalletSource::Mnemonic(mnemonic.to_string())).unwrap();         assert_eq!(wallet.network.id, network.id);
//         assert_eq!(wallet.network.display_name, network.display_name);
//         assert!(!wallet.id.is_empty());
//         assert!(!wallet.address.address_id.is_empty());
...
//         let wallet =
//             LocalEthersWallet::recover_wallet(network.clone(),
// WalletSource::Mnemonic(mnemonic.to_string())).unwrap();
