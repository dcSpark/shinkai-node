...
use shinkai_node::shinkai_message::encryption::{
    hash_encryption_public_key, encryption_public_key_to_string, unsafe_deterministic_encryption_keypair,
};
use shinkai_node::shinkai_message::shinkai_message_handler::ShinkaiMessageHandler;
use shinkai_node::shinkai_message::signatures::{unsafe_deterministic_signature_keypair, signature_public_key_to_string};
...
fn tcp_node_test() {
    ...
    rt.block_on(async {
        ...
        let mut node1 = Node::new(node1_identity_name.to_string(), addr1, node1_identity_sk, node1_encryption_sk, 0, node1_commands_receiver, node1_db_path);

        let addr2 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 8081);
        let mut node2 = Node::new(node2_identity_name.to_string(), addr2, node2_identity_sk, node2_encryption_sk, 0, node2_commands_receiver, node2_db_path);
