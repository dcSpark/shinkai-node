...
describe('Errors Exec', () => {
  test('fill-memory', async () => {
    ...
    expect(x).toEqual({"result": {"error": "Worker terminated due to reaching memory limit: JS heap out of memory",}, "tool": "ErrorGenerator"});
  }, 60000);
});
...
describe('Errors Exec', () => {
  test('terminate', async () => {
    ...
    const x = await execMode(data.source, data.tool, JSON.stringify(data.input), JSON.stringify(data.headers || {}));
    expect(x).toEqual({"result": {"errorCode": 1,}, "tool": "ErrorGenerator"});
  }, 60000);
});
...
describe('Errors Exec', () => {
  test('throw-exception', async () => {
    ...
    const x = await execMode(data.source, data.tool, JSON.stringify(data.input), JSON.stringify(data.headers || {}));
    expect(x).toEqual({"result": {"error": "ErrorGenerator: throw-exception",}, "tool": "ErrorGenerator"});
  }, 60000);
});
