...
impl WasmEncryptionMethod {
    ...
    pub fn from_str(s: &str) -> Self {
        ...
        WasmEncryptionMethod { value }
    }
}

pub fn wasm_unsafe_deterministic_encryption_keypair(n: u32) -> (StaticSecret, PublicKey) {
...
pub fn wasm_ephemeral_encryption_keys() -> (StaticSecret, PublicKey) {
    #[allow(deprecated)]
...
pub fn wasm_encryption_secret_key_to_string(secret_key: StaticSecret) -> String {
    let bytes = secret_key.to_bytes();
    bs58::encode(&bytes).into_string()
}

pub fn wasm_encryption_public_key_to_string(public_key: PublicKey) -> String {
    let bytes = public_key.to_bytes();
    bs58::encode(&bytes).into_string()
}

pub fn wasm_encryption_public_key_to_string_ref(public_key: &PublicKey) -> String {
    wasm_encryption_public_key_to_string(public_key.clone())
}

pub fn wasm_string_to_encryption_static_key(encoded_key: &str) -> Result<StaticSecret, &'static str> {
...
pub fn encrypt_body(
    ...
    let encryption_method = WasmEncryptionMethod::from_str(encryption);
    if encryption_method.as_str() == "default" {
...
pub fn decrypt_body_message(
    ...
    let encryption_method = WasmEncryptionMethod::from_str(message.encryption.as_str());
    if encryption_method.as_str() == "default" {
...
pub fn decrypt_content_message(
    ...
    if encryption_method.as_str() == "default" {
        ...
        Err(DecryptionError::new("Encryption method is None"))
    }
}
