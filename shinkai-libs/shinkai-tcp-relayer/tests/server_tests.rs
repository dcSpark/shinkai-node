...
#[tokio::test]
async fn test_message_from_localhost_to_nico_requester_auto() {
    // Setup a TCP listener
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    ...
    let payload = create_shinkai_message_for_shared_files(
        "@@localhost.sepolia-shinkai",
        "@@hackernews_com.sepolia-shinkai",
        "main",
    )
    .await;
    ...
    let encoded_msg = create_shinkai_message(
        "@@localhost.sepolia-shinkai",
        "@@hackernews_com.sepolia-shinkai",
        "Message from localhost to hackernews",
    )
    .await;
...
// #[tokio::test]
async fn test_message_from_nico_requester_to_localhost() {
    // Setup a TCP listener
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
