...
sudo docker run -it -d --name $CONTAINER_NAME -v ~/shinkai/develop/shinkai-node/shinkai-libs:/project pyo3-maturin:latest /bin/bash
...
while [[ $file_exists -eq 0 && $elapsed_time -lt $timeout ]]; do
    ...
    if sudo docker exec $CONTAINER_NAME test -e "/project/shinkai-message-pyo3/target/wheels/shinkai_message_pyo3-$version-cp38-abi3-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"; then
        file_exists=1
    else
        sleep $interval
...
if [[ $file_exists -eq 1 ]]; then
    sudo docker cp $CONTAINER_NAME:/project/shinkai-message-pyo3/target/wheels/shinkai_message_pyo3-$version-cp38-abi3-manylinux_2_17_x86_64.manylinux2014_x86_64.whl /home/nico/ai-dcspark/development/shinkai-node/shinkai-libs/
