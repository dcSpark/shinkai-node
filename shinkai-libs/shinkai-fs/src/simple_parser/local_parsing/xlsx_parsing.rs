...
use super::{xlsx_tool::parse_xlsx_to_matrix, LocalFileParser};
...
impl LocalFileParser {
    ...
    pub async fn process_xlsx_file_async(
        file_path: PathBuf,
        ...
        let xlsx_lines = xlsx_lines.into_iter().map(|row| row.join("|")).collect::<Vec<String>>();
        Self::process_table_rows(xlsx_lines, max_node_text_size)
    }
...
mod tests {
    ...
    fn read_test_file(filename: &str) -> Vec<u8> {
        let path = path::absolute(Path::new("src/test_data/test.xlsx"))
            .unwrap()
            .to_path_buf();
