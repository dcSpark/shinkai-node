...
impl RelayManager {
    ...
    pub fn register_peer(&mut self, identity: String, peer_id: PeerId) {
        println!(
            "🔄 Peer {} registered with PeerId: {} - will update peer discovery information",
            identity, peer_id
        );
        ...
        if let Some(existing_peer_id) = self.registered_peers.get(&identity) {
            ...
            if existing_peer_id != new_peer_id {
                ...
                if self.swarm.is_connected(&existing_peer_id) {
                    println!(
                        "🔄 Disconnecting stale peer {} to allow new peer {} for identity {}",
    ...
    async fn handle_swarm_event(&mut self, event: SwarmEvent<RelayBehaviourEvent>) -> Result<(), LibP2PRelayError> {
        match event {
            ...
            SwarmEvent::Behaviour(RelayBehaviourEvent::Identify(IdentifyEvent::Received { peer_id, info, .. })) => {
                println!(
                ...
                if let Some(existing_identity) = self.find_identity_by_peer(&peer_id) {
                    println!(
                        "🔄 Peer {} already registered with identity: {} - skipping re-identification",
                        ...
                        ed25519_dalek::VerifyingKey::from_bytes(&key_bytes.try_into().unwrap_or([0u8; 32]))
                    {
                            ...
                            .await
                        {
                            ...
                            if let Some(identity) = possible_identity {
                                println!(
                                    "❌ Verification failed, registering peer {} with identity: {}",
                                    peer_id, identity
                                );
                        ...
                        match message {
                            request_response::Message::Request {
                                ...
                                if request.external_metadata.sender.starts_with("@@localhost.") {
                                    println!(
                                        "🔄 Relay: We need to re-encrypt message from localhost to {}",
                                        target_node
                                    );
                                    // Capture the localhost encryption public key stored in `other`
                                    let original_sender_enc_key = request.external_metadata.other.clone();
                                    ...
                                    // Move encryption key to intra_sender and clear the other field
                                    request.external_metadata.intra_sender = original_sender_enc_key;
                                    request.external_metadata.other = String::new();
