...
mod tests {
    ...
    // IMPORTANT:
    // The tokenization algo implemented doesn't match the specs so the tests WILL FAIL
    // why bother then? it's used just as an approximation

    fn test_case(input_string: &str, expected_token_ids: Vec<i32>) {
        let load_start = std::time::Instant::now();
        let tokenizer = LlamaTokenizer::new();
        let load_duration = load_start.elapsed();
        eprintln!("Time taken to load LlamaTokenizer: {:?}", load_duration);
    
        let encode_start = std::time::Instant::now();
        let alt_tokens = tokenizer.encode(input_string);
        let encode_duration = encode_start.elapsed();
    
        eprintln!("alt_tokens: {:?}", alt_tokens);
        eprintln!("length of alt_tokens: {}", alt_tokens.len());
        eprintln!("Time taken for encoding: {:?}", encode_duration);
    
        let decode_start = std::time::Instant::now();
        let dec_tokens = tokenizer.decode(alt_tokens);
        let decode_duration = decode_start.elapsed();
    
        eprintln!("dec_tokens: {:?}", dec_tokens);
        eprintln!("Time taken for decoding: {:?}", decode_duration);
    }
